...
const BASES = [
	{ locale: 'root', dir: 'src/content/docs/samhitas/krishna-yajur/taittiriya-samhita' },
	{ locale: 'iast', dir: 'src/content/docs/iast/samhitas/krishna-yajur/taittiriya-samhita' },
];
...
function renderKandaIndex(kanda, locale) {
	...
	return [
		...
		'',
		`<TaittiriyaKanda kanda={${kanda}} locale="${locale}" />`,
		'',
	].join('\n');
}
...
function renderPrapathakaIndex(kanda, prapathaka, locale) {
	...
	return [
		...
		'',
		`<TaittiriyaChapter chapter={${chapter}} locale="${locale}" />`,
		'',
	].join('\n');
}
